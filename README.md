<p align="center">
  <img src="brand/png/light/frontpost-logo-light-1024.png" alt="FrontPost logo" width="160" />
</p>
...
> 当前仓库是 FrontPost Design 发布站点。完整业务产品建议拆成 `frontend/` 与 `backend/` 两个主目录；设计系统已经是独立库，本业务仓库不再保留 `packages/design-system`。详见 [生产级 Monorepo 初步框架](docs/repo-blueprint.md)。

核心原则：

- `frontend/` 统一承载 Web、Expo Mobile 与预留 Desktop，Expo 下继续区分 `shared/`、`ios/`、`android/`。
- `backend/` 统一承载 FastAPI API、RQ/Redis Worker、Scheduler、Crawler 与后端共享模块。
- 定时任务与爬虫采用 Scheduler 入队、Worker 执行、Redis 队列削峰的模式，避免 HTTP API 被长耗时任务拖慢。
- Docker / K8s 从第一天按可运行部件拆镜像与 Deployment：Web、API、Worker、Scheduler、Crawler。
- 所有进程启动时打印统一 FrontPost banner，例如 `FrontPost-Frontend-Web`，便于 CI/CD 与多控制台排障。

### 启动 Banner

```bash
./scripts/frontpost-banner.sh FrontPost-Frontend-Web
./scripts/frontpost-banner.sh FrontPost-Backend-API
```
